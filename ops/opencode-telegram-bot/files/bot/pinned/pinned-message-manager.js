...
import { getCurrentProject, getPinnedDashboardEnabled, getPinnedMessageId, setPinnedMessageId, clearPinnedMessageId, } from "../../app/stores/settings-store.js";
...
class PinnedMessageManager {
    ...
    async refreshProjectMetadata() {
        const project = getCurrentProject();
        this.state.projectPath = getProjectDisplayName(project?.worktree) || project?.worktree || t("pinned.unknown");
        ...
        try {
            ...
            this.state.projectPath = getProjectDisplayName(project.worktree) || worktreeContext.mainProjectPath;
    ...
    formatMessage() {
        ...
        const modelName = formatModelDisplayName(currentModel.providerID, currentModel.modelID, currentModel.variant);
    ...
    async applyPinnedDashboardEnabled(enabled) {
        if (!enabled) {
    ...
    async unpinMessageOrThrow(messageId) {
        if (!this.api || !this.chatId) {
            throw new Error("Pinned manager is not initialized");
        }
        try {
    ...
    hydrateDashboardFromLocalState() {
    ...
    async dropOwnedDashboardQuietly() {
        const messageId = this.state.messageId ?? this.leftoverUnpinMessageId;
        if (!messageId) {
            return;
        }
    ...
    parkLeftoverDashboard(messageId) {
        this.leftoverUnpinMessageId = messageId;
        this.state.messageId = null;
        this.lastRenderedMessageText = null;
    }
    isMessageMissingError(err) {
        const errorMessage = err instanceof Error ? err.message.toLowerCase() : String(err).toLowerCase();
        return (errorMessage.includes("message to edit not found") ||
